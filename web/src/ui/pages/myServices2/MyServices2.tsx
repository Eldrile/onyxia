...
import { MyServices2ButtonBar } from "./MyServices2ButtonBar";
import { MyServices2Cards } from "./MyServices2Cards";
import type { Props as MyServices2CardsProps } from "./MyServices2Cards";
import {
    MyServicesRestorableConfigs,
    type Props as MyServicesRestorableConfigsProps
} from "../myServices/MyServicesRestorableConfigs";
import { ButtonId } from "./MyServices2ButtonBar";
...
import {
    ...
    type Props as MyServices2ConfirmDeleteDialogProps
} from "./MyServices2ConfirmDeleteDialog";
...
export default function MyServices2(props: Props) {
    ...
    /* prettier-ignore */
    const { serviceManagement2, restorableConfigManagement, k8sCodeSnippets } = useCore().functions;
    /* prettier-ignore */
    const { restorableConfigs, chartIconAndFriendlyNameByRestorableConfigIndex } = useCoreState("restorableConfigManagement", "main");
    const isUpdating = useCoreState("serviceManagement2", "isUpdating");
    const runningServices = useCoreState("serviceManagement2", "runningServices");
    ...
    const commandLogsEntries = useCoreState("serviceManagement2", "commandLogsEntries");
    ...
    const onButtonBarClick = useConstCallback(async (buttonId: ButtonId) => {
        switch (buttonId) {
            case "launch":
                routes.catalog2().push();
                return;
            case "refresh":
                serviceManagement2.update();
                return;
            case "trash":
                ...
                deletableRunningServiceHelmReleaseNames.map(helmReleaseName =>
                    serviceManagement2.stopService({ helmReleaseName })
                );
    ...
    useEffect(() => {
        const { setInactive } = serviceManagement2.setActive();
        return () => setInactive();
    }, []);
    ...
    const cards = useMemo(
        (): MyServices2CardsProps["cards"] | undefined =>
            runningServices?.map(
                ({
                    helmReleaseName,
                    chartIconUrl,
    ...
    const evtMyServiceCardsAction = useConst(() =>
        Evt.create<UnpackEvt<MyServices2CardsProps["evtAction"]>>()
    );
    ...
    const catalogExplorerLink = useMemo(() => routes.catalog2().link, []);

    const evtConfirmDeleteDialogOpen = useConst(() =>
        Evt.create<UnpackEvt<MyServices2ConfirmDeleteDialogProps["evtOpen"]>>()
    );

    const onRequestDelete = useConstCallback<MyServices2CardsProps["onRequestDelete"]>(
    ...
    return (
        <div className={cx(classes.root, className)}>
            ...
            <div className={classes.belowHeader} ref={belowHeaderRef}>
                    ...
                    <CommandBar
                        ...
                        helpDialog={{
                            "body": tCatalogLauncher("api logs help body", {
                                ...
                                "myServicesHref": routes.myServices2().href,
                    ...
                    <>
                        {!isSavedConfigsExtended && (
                            <MyServices2Cards
                                ...
                                getEnv={serviceManagement2.getEnv}
                                getPostInstallInstructions={
                                    serviceManagement2.getPostInstallInstructions
                                }
                            />
                        )}
                        <MyServicesRestorableConfigs
                ...
                <MyServices2ConfirmDeleteDialog evtOpen={evtConfirmDeleteDialogOpen} />
            </div>
        </div>
    );
}
