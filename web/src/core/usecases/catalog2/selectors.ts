...
const catalogs2 = createSelector(readyState, state => {
    if (state === undefined) {
        return undefined;
    }
    return state.catalogs2;
});
...
const selectedCatalog = createSelector(
    ...
    catalogs2,
    selectedCatalogId,
    (isReady, catalogs2, selectedCatalogId) => {
        if (!isReady) {
            return undefined;
        }
        ...
        const catalog = catalogs2.find(({ id }) => id === selectedCatalogId);
...
const availableCatalogs = createSelector(
    catalogs2,
    (catalogs2): { catalogId: string; catalogName: LocalizedString }[] | undefined => {
        if (catalogs2 === undefined) {
            return undefined;
        }
